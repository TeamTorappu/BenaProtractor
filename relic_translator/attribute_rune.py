...
def rogue_char_attribute_mul(item_type,blackboard):
    ...
    return {"main" : f"{timing}所有{selector}{'、'.join(modifiers)}（藏品符文）"}
...
def rogue_layer_char_attribute_mul(item_type,blackboard):
    ...
    if "stack_by_res" in blackboard:
        ...
        selector = analyze_selector(blackboard,"我方")
        return {
            "main" : f"{timing}每有{need_var}个 {need_item.display_name}（向下取整），{selector}{'、'.join(modifiers)}（藏品符文）",
            "link" : "rogue_item." + need_item_key
        }
    else: # 普通叠层
        # 选择器目标处理
        selector = analyze_selector(blackboard,"我方")
        return {"main" : f"{timing}每有一层叠加层数，所有{selector}{'、'.join(modifiers)}（藏品符文）"}
